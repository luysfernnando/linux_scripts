...
| Windows Terminal | fonte `JetBrainsMono NF` (variante não-Mono: ícones na largura de design, maiores que no `NFM`) + `builtinGlyphs: false` (sem isso glifos Nerd Font do starship quebram), opacity 80 | `ricing/terminal/windows-terminal/settings.json` | Sim (`%LOCALAPPDATA%\Packages\Microsoft.WindowsTerminal_8wekyb3d8bbwe\LocalState\settings.json`) |
...
| Glifos Nerd Font quebrados no Windows Terminal (separadores powerline como bloco, ícone de SO tofu, relógio ausente) | fonte do perfil não é Nerd Font, ou `builtinGlyphs: true` faz o WT desenhar box-drawing com glifos próprios | `terminal/windows-terminal/settings.json`: `profiles.defaults.font.face = "JetBrainsMono NF"` + `builtinGlyphs: false`. Fonte do WT vem só do settings.json dele — `wezterm.lua` não afeta as abas do Windows Terminal. Trocar de fonte exige fechar o WT inteiro, não só abrir aba nova (cache por processo) |
| Ícones do fastfetch pequenos, ou de tamanhos diferentes entre si | `NFM` (Mono) comprime todo glifo em 1 célula: uniforme mas pequeno. `NF` (não-Mono) respeita a largura de design: maior, porém desigual, porque cada ícone vem de um set patchado diferente (Material Design, Font Awesome, Octicons, Seti) com grid próprio | Escolha, não bug — `NF` pra ícone grande, `NFM` pra uniforme. Uniforme **e** grande só normalizando os glifos na mão: `display.key.type = "string"` + `"key"` explícito por módulo, todos do mesmo set. Custo: o ícone de SO deixa de variar por plataforma, e o `.tmpl` é compartilhado com Linux/WSL. Não existe knob de tamanho por codepoint no WT |
| Logo do fastfetch invisível no Windows nativo (`Image Magick library not found`, depois `Failed to load / convert the image source`) | fastfetch dlopen `libMagickCore-7.Q16HDRI-10.dll` (naming MSYS2, ABI 7.1.1); instalador oficial entrega `CORE_RL_MagickCore_.dll` 7.1.2 — renomear muda o erro, não resolve (ABI difere). Montar DLLs mingw do 7.1.1 dá `WinError 127` | Converter fora do fastfetch: `chafa` gera `~/.config/fastfetch/165.sixel` e `logo.type = raw` despeja byte a byte (`render_sixel_logo` no `install-menu.sh`). Cobre Windows Terminal e WezTerm. **Nunca `file-raw`** — lê como linhas de texto, injeta escape no meio do blob DCS, tela vira listras. `width`/`height` de `raw` são só reserva de célula: se não casarem com o sixel, o texto sobrepõe a imagem ou sobra vão em branco |
| Logo do fastfetch com borda branca serrilhada, ou fundo opaco em terminal translúcido, ou cheio de chuvisco | Sixel não tem canal alpha. ImageMagick achata transparência numa cor (branco por padrão; achatar na cor do terminal troca a borda por retângulo opaco). No chafa, `--colors 256` = paleta FIXA do xterm (cubo 6x6x6, nenhuma cor da imagem), não "256 cores" | Converter com `chafa --colors full --dither none`: `full` monta paleta a partir da imagem, e o `P2=1` do header DCS deixa pixel não pintado transparente de verdade. Conferir decodificando o blob — chafa pinta ~60% da área, ImageMagick pinta 100% |
