...
**Modo global (evolução semanal):** varre `~/.agents/rules/common`, `~/.agents/skills` e repos conhecidos, procurando regra que deveria ser global (comportamento genérico do Claude, não domínio do projeto) e duplicação entre repo×global. Estado da última rodada fica em memory (`reference`, projeto `linux_scripts`) — ver seção "Modo global" do `process.md`.
